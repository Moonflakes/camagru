...
function set_pict($num_pg, $nb_img_pg)
{
    include_once '../config/setup.php';
    include_once 'check_user.php';

    session_unset($_SESSION['pictures']);
    ...
    if ($pictinfo = $req->fetchall())
    {
        ...
        foreach ($pictinfo as $key => $array) 
        {
            ...
            $_SESSION['pictures']['pict_'.++$i] = array('p_id' => $id, 'p_auth' => $auth, 'p_date' => $date, 'p_path' => $path, 'p_descr' => $descr, 'p_nblike' => $nblike, 'p_nbcom' => $nbcom, 'p_like' => $like);
        }
    }
}
?>